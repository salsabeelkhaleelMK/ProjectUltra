    ...
    <div class="space-y-6 max-h-[75vh] overflow-y-auto px-2">
      ...
      <div>
        ...
        <div class="mb-4">
          <div class="relative">
            ...
            <input
              ...
              class="w-full bg-white border border-gray-200 rounded-lg pl-10 pr-3 py-2 text-sm focus:outline-none focus:border-blue-500 transition-colors"
            />
          </div>
        </div>
        
        <!-- Stock Vehicles Grid -->
        <div v-if="filteredStock.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          <VehicleCard
        ...
        <div v-else class="text-center py-8 text-gray-500">
          ...
          <p class="text-xs mt-1 text-gray-400">Try different keywords or clear the search</p>
        </div>
      </div>
...
const allRecommendedVehicles = computed(() => {
  ...
  // Add requested vehicle first if it's in stock
  if (hasRequestedInStock.value) {
    vehicles.push({
  ...
  if (mockVehicles && mockVehicles.length > 0) {
    ...
    // Add first 3 alternatives (or 4 if no requested vehicle)
    const limit = hasRequestedInStock.value ? 3 : 4
